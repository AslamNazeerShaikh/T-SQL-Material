...
-- Match tests
SELECT * FROM #Cust15 WHERE Id IN (SELECT CustId FROM #Ord15 WHERE CustId IS NOT NULL);
SELECT c.* FROM #Cust15 c WHERE EXISTS (SELECT 1 FROM #Ord15 o WHERE o.CustId = c.Id);
...
SELECT c.* FROM #Cust15 c LEFT JOIN #Ord15 o ON o.CustId = c.Id WHERE o.Id IS NULL;
SELECT * FROM #Cust15 WHERE Id NOT IN (SELECT CustId FROM #Ord15);  -- EMPTY (NULL trap)
