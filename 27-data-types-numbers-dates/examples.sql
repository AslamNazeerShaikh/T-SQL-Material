...
SELECT GETDATE() AS OldLocal, SYSDATETIME() AS Sharp, SYSUTCDATETIME() AS Utc,
       CAST(GETDATE() AS DATE) AS JustDay, CAST(GETDATE() AS TIME) AS JustTime;
...
SELECT IDENT_CURRENT('dbo.Key27') AS AnyScope,  -- any scope/sitting peek
       IDENT_INCR('dbo.Key27') AS Step, IDENT_SEED('dbo.Key27') AS Seed;
GO
...
SELECT @X.value('(/r/i)[1]', 'VARCHAR(20)') AS First,
