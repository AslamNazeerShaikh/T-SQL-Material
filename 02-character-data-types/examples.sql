...
IF
    OBJECT_ID('dbo.DemoStrings_02', 'U') IS NOT NULL
...
SELECT
    @Good AS Unicode_OK,
    @Bad AS NonUnicode_Mangled;

-- CHAR padding behavior
SELECT
    DATALENGTH(Code) AS CodeBytes,
    LEN(Code) AS CodeLen
FROM dbo.DemoStrings_02;
...
SELECT *
FROM dbo.DemoStrings_02
WHERE Name = 'sara' COLLATE Latin1_General_CS_AS;
-- Sara row
SELECT *
FROM dbo.DemoStrings_02
WHERE Name = 'sara' COLLATE Latin1_General_CI_AS;
